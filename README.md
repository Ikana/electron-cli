...
- `package`: copies the installed Electron runtime, app files, installed production dependency closure by default, app metadata, configured app icons, and extra resources into a local app bundle for the current platform and architecture; it reads package metadata, dependency pruning, ignore rules, ASAR settings, and Windows executable metadata from `package.json`, JSON-shaped Forge config, and static Forge config files, can embed Windows executable icons, and can apply experimental Rust-native ad-hoc macOS bundle signatures.
- `make`: runs `package` and writes distributables under `out/make/<target>/<platform>/<arch>/`; it reads JSON-shaped `config.forge.makers` / `electronCli.makers` arrays and static Forge config files when `--target` is omitted, and `--target` still forces one maker. ZIP works on all platforms, `--target dmg` writes a basic macOS disk image, `--target deb` / `--target rpm` write Linux packages, and `--target msi` writes a Windows Installer package.
...
The DMG maker is currently a pure-Rust FAT32 image with the app bundle and an Applications entry. The MSI maker writes a compressed embedded CAB, Windows Installer database tables, and a Start Menu shortcut when the packaged executable is present; configured `maker-wix` values for `name`, `version`, `manufacturer`, `description`, `exe`, `icon`, `shortName`, `appUserModelId`, `toastActivatorClsid`, `language`, `programFilesFolderName`, `shortcutFolderName`, `shortcutName`, `upgradeCode`, `installLevel`, `rebootMode`, `defaultInstallMode`, `associateExtensions`, `autoRun`, and `features.autoLaunch` are reflected in the MSI metadata, install directories, install scope, embedded icon, shortcut identity, shortcut target, post-install launch action, launch-on-login registry entry, and file association tables. Deb/RPM makers install a configured Linux icon into `/usr/share/pixmaps` and point the desktop entry at it. HFS+/APFS DMG layout customization, ASAR transform options, installer UI customization, macOS keychain signing, and additional notarization auth modes are still TODO.
...
```json
{
  ...
  "electronCli": {
    "packagerConfig": {
      ...
      "helperBundleId": "com.example.my-app.helper",
      ...
      "prune": true,
      "asar": {
        "ordering": "assets/asar-ordering.txt",
        "unpack": "**/*.node",
    ...
    "makers": [
      ...
      { "name": "@electron-forge/maker-rpm", "platforms": ["linux"], "config": { "options": { "icon": "assets/icon.png" } } },
      { "name": "@electron-forge/maker-wix", "platforms": ["win32"], "config": { "manufacturer": "Example Inc", "exe": "my-app.exe", "language": 1033, "shortcutFolderName": "Example Inc", "installLevel": 2, "defaultInstallMode": "perUser", "associateExtensions": "thing,work", "autoRun": true, "features": { "autoLaunch": { "enabled": true, "arguments": ["--hidden"] } } } }
    ],
    "publishers": [
...
The package command also reads JSON-shaped `config.forge.packagerConfig` and `electronPackagerConfig` entries for the same fields. On macOS, `packagerConfig.extendInfo` can be an object or plist file path merged into the main app `Info.plist` before explicit options such as `appBundleId`; `helperBundleId` and `extendHelperInfo` apply equivalent metadata to Electron helper apps, and version / usage description metadata is propagated to helpers. `protocols` writes `CFBundleURLTypes` and `usageDescription` writes `NS*UsageDescription` entries. On Windows, `packagerConfig.icon` resolves to an `.ico` file and is embedded into the renamed `.exe` as PE icon resources; `appVersion`, `buildVersion`, `appCopyright`, package `author`, and `packagerConfig.win32metadata` write PE version-info strings such as `CompanyName`, `FileDescription`, `InternalName`, `OriginalFilename`, and `ProductName`. Windows `win32metadata.application-manifest` embeds a local manifest file, and `win32metadata.requested-execution-level` rewrites the default manifest execution level to `asInvoker`, `highestAvailable`, or `requireAdministrator`; those two options are mutually exclusive, matching Electron Packager's resedit flow. `packagerConfig.prune` defaults to `true`, copying only the installed runtime dependency closure; `prune: false` copies installed `node_modules` into the app while still applying Packager-style default ignores such as lockfiles, `node_modules/.bin`, object files, and `node_gyp_bins`. `packagerConfig.asar: true` writes `resources/app.asar` natively in Rust, records Electron-compatible ASAR integrity metadata, embeds the packaged ASAR header hash into macOS `Info.plist` or Windows PE resources, and removes the loose `resources/app` staging directory before signing or making artifacts. ASAR option objects support `ordering` files, `unpack` file globs, and `unpackDir` directory globs/prefixes, writing matched content to `resources/app.asar.unpacked`; `transform` and other ASAR options are reported as not implemented yet. `packagerConfig.ignore` accepts regex strings or JavaScript-style regex literal strings such as `"/^\\/coverage/i"`; patterns are matched against project-relative paths with and without a leading slash, plus the absolute source path. The ignore filter applies to copied app files and copied `node_modules` content, but not to the Electron runtime, icons, or explicit `extraResource` entries. The make command maps JSON-shaped Forge maker names to the Rust-native targets it supports: zip, dmg, deb, rpm, and wix/msi. Deb/RPM makers read Forge-style `config.options.icon` / `config.icon` and fall back to `packagerConfig.icon` when installing Linux desktop icons; the wix/msi maker reads Forge-style `config.name`, `config.version`, `config.manufacturer`, `config.description`, `config.exe`, `config.icon`, `config.shortName`, `config.appUserModelId`, `config.toastActivatorClsid`, `config.language`, `config.programFilesFolderName`, `config.shortcutFolderName`, `config.shortcutName`, `config.upgradeCode`, `config.installLevel`, `config.rebootMode`, `config.defaultInstallMode`, `config.associateExtensions`, `config.autoRun`, and `config.features.autoLaunch`. The publish command maps JSON-shaped publisher names to local and GitHub.
