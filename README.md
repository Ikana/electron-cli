...
The DMG maker is currently a pure-Rust FAT32 image with the app bundle and an Applications entry. The MSI maker writes a compressed embedded CAB, Windows Installer database tables, and a Start Menu shortcut when the packaged executable is present; configured `maker-wix` values for `name`, `version`, `manufacturer`, `description`, and `exe` are reflected in the MSI metadata and shortcut target. Deb/RPM makers install a configured Linux icon into `/usr/share/pixmaps` and point the desktop entry at it. HFS+/APFS DMG layout customization, ASAR transform options, installer UI customization, macOS keychain signing, and additional notarization auth modes are still TODO.
...
```json
{
  ...
  "electronCli": {
    ...
    "makers": [
      ...
      { "name": "@electron-forge/maker-wix", "platforms": ["win32"], "config": { "manufacturer": "Example Inc", "exe": "my-app.exe" } }
    ],
    "publishers": [
